...
// Compute the scores for each digit from a batch of input images.
export function predictSoft(x) {
  const pred = tf.tidy(() => {
    return model(x);
  });
  return pred;
...
export class MICP {
  constructor(calibrScores,calibrLabels) {
    this.calibrScores = calibrScores;
    this.calibrLabels = calibrLabels;
  }

  // This computes pValues for 1 test object
  // testScores is a (1 x #labels) tensor
  pValues1d(testScores) {
    let ls = tf.less(this.calibrScores,testScores).logicalAnd(this.calibrLabels);
    let eq = tf.equal(this.calibrScores,testScores).logicalAnd(this.calibrLabels);
    let smoothers = tf.randomUniform(testScores.shape);

    let eqTerm = eq.sum(0).add(1).toFloat().mul(smoothers);

    let num = ls.sum(0).toFloat().add(eqTerm);
    let den = this.calibrLabels.sum(0).add(1).toFloat();
    let pVals = num.div(den);
    return pVals;
  }

  pValues(testScores) {
    let ts = Array();
    for (let i=0; i<testScores.shape[0]; i++) {
      ts.push(this.pValues1d(testScores.slice([i],[1])).as1D());
    }
    return tf.stack(ts,0);
  }
}
